...
from Database import Database, Person, Room
...
class Dojo:
    ...
    people = []  # list of people in the Dojo
    ...
    def create_livingspace(self, name):
        """Create a new livingspace in Dojo"""
    ...
    def allocate_livingspace(self, name):
        ...
        if not rooms:
            return colored(
                'There are no available livingspaces to allocate to {}!'
                .format(name), 'white', 'on_red')
        else:
            room = random.choice(rooms)
            room['occupants'].append(name)
            return colored(
                'Fellow {} has been allocated space in livingspace {}.'
    ...
    def get_room(self, room_type=None, room_name=None, available_rooms=None,
                 person=None):
        """Get a list of specific rooms in the Dojo depending on room type, name
    ...
    def check_room_availability(self, room_name):
        """Check for the availability of a specific room in Dojo"""
        if self.room_exists(room_name):
    ...
    def person_exists(self, name):
        """Check if a person exists in Dojo"""
        if self.people:
    ...
    def remove_person_from_room(self, person_name, office=None,
                                livingspace=None):
        '''Remove a person from all assigned rooms in Dojo'''
        if office:
    ...
    def remove_person(self, name):
        '''Remove a person completely from Dojo'''
        try:
    ...
    def print_unallocated(self, filename=None):
        """Print out the unallocations in Dojo"""
        allocated = {}
        for room in self.rooms:
    ...
    def print_room(self, room_name):
        """Print the people in a specific room in Dojo"""
        try:
            if self.room_exists(room_name):
                people = [
                ...
                if not people:
                    ...
                    return ', '.join(people)
            raise ValueError('No such room as {} in Dojo!'.format(room_name))
        except ValueError:
            raise

    def add_fellow(self, person_name, wants_livingspace):
        '''Add a fellow to Dojo'''
    ...
    def save_state(self, database=None):
        """Save the current state of Dojo to the database"""
        self.database.clear()
        self.database.initialize(database)
        print(self.save_people())
        print(self.save_rooms())
        self.database.save()
        return colored("Dojo's state saved successfully.", 'white',
                       'on_green')

    def save_rooms(self):
        '''Save the current state of rooms in Dojo to the database'''
        for room in self.rooms:
        ...
        return colored('Rooms in Dojo saved successfully.', 'white',
                       'on_green')
    ...
    def save_people(self):
        ...
        return colored('People in Dojo saved successfully.',
                       'white', 'on_green')

    def load_state(self, database=None):
        """Load Dojo resources from the database"""
        self.database.initialize(database)
        Dojo_rooms = self.database.session.query(Room).all()
        print(self.rooms_load(Dojo_rooms))
        people = self.database.session.query(Person).all()
        print(self.people_load(people))
        return colored("Dojo's previous state loaded successfully.", 'white',
                       'on_green')

    def rooms_load(self, Dojo_rooms):
        '''Load rooms from the database to Dojo'''
        for room in Dojo_rooms:
            room_dict = {
                'name': room.name,
                'room_type': room.type,
                'max_space': room.max_space,
